...
# For all numbers to draw
for index, number in enumerate(numbers):
    # First mark all corresponding numbers with -1
    ...
    # For every board, check if completed
    for i, board in enumerate(boards):
        for line in board:
            # I'm checking columns only
            # I knew it wasn't a row through trial and error, not very scientific 8-)
            for n in range(5):
                sum = 0
                for line in board:
                    sum += int(line[n])
                if sum == -5:
                    ...
                    break
                # Look son, how beautiful this Christmas tree is
                else:
                    continue
                break
            else:
...
output = open("day_4_output_part_1", "w")
